 ...
 * the user taps them, or as `pyrx:push:received-cold-start` when the
 * tap launches the app from terminated state.)
 *
 ...
 *
 * Available since 0.2.0 (the native observer surface that drives this
 * hook landed in `PYRXSynapse 0.1.2` / `tech.pyrx.synapse:synapse-{core,push}:0.1.4`).
 * In 0.1.x this hook registered a listener but the callback never fired;
 * in 0.2.0 it works as documented.
 */
